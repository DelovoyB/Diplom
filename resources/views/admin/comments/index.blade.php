...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="w-full text-center lg:text-left text-gray-700 text-3xl font-medium">Отзывы</h3>
            ...
            <div class="pag flex flex-col items-center -my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200 mb-2">
                    <div class="grid min-w-full grid-cols-[repeat(16,_minmax(0,_1fr))]">
                        ...
                        <a href="{{ route("admin.comments.sort", ["title", isset($direction) && $direction == "asc" && $column == "title" ? "desc" : "asc"]) }}" class="flex items-center justify-center md:justify-start col-span-7 md:col-span-3 xl:col-span-4 md:px-6 text-center md:text-left py-3 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>Товар</span>
                            @if(isset($direction) && isset($column) && $column == "title")
                                <svg class="w-3 h-3 @if($direction == "desc") rotate-180 @endif" fill="#6b7280" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M21,21H3L12,3Z"></path></g></svg>
                            @endif
                        </a>
                        <a href="{{ route("admin.comments.sort", ["name", isset($direction) && $direction == "asc" && $column == "name" ? "desc" : "asc"]) }}" class="hidden md:flex items-center md:col-span-3 xl:col-span-4 px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>Пользователь</span>
                            @if(isset($direction) && isset($column) && $column == "name")
                                <svg class="w-3 h-3 @if($direction == "desc") rotate-180 @endif" fill="#6b7280" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M21,21H3L12,3Z"></path></g></svg>
                            @endif
                        </a>
                        <a href="{{ route("admin.comments.sort", ["text", isset($direction) && $direction == "asc" && $column == "text" ? "desc" : "asc"]) }}" class="hidden md:flex items-center md:col-span-5 xl:col-span-4 px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>Текст</span>
                            @if(isset($direction) && isset($column) && $column == "text")
                                <svg class="w-3 h-3 @if($direction == "desc") rotate-180 @endif" fill="#6b7280" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M21,21H3L12,3Z"></path></g></svg>
                            @endif
                        </a>
                        <div class="col-span-7 md:col-span-3 xl:col-span-2 text-right px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route("admin.comments.create") }}" class="text-blue-700 hover:text-blue-500">Добавить</a>
                        </div>
                        @foreach($comments as $comment)
                            <div class="odd:bg-white even:bg-slate-50 text-sm leading-5 text-gray-900 col-[1_/_span_16] grid grid-cols-[repeat(16,_minmax(0,_1fr))]">
                                <div class="col-span-2 md:col-span-2 md:px-6 text-center md:text-left py-4 font-medium">{{ $comment->id }}</div>
                                <div class="col-span-7 md:col-span-3 xl:col-span-4 md:px-6 text-center md:text-left py-4 font-medium truncate">{{ $comment->title }}</div>
                                <div class="hidden md:block md:col-span-3 xl:col-span-4 px-6 py-4">{{ $comment->name }}</div>
                                <div class="hidden md:block md:col-span-5 xl:col-span-4 px-6 py-4">{{ $comment->text }}</div>
                                <div class="col-span-7 md:col-span-3 xl:col-span-2 px-6 py-4 text-right flex flex-row md:flex-col justify-end gap-2">
                                    <a href="{{ route("admin.comments.edit", $comment->id) }}" class="text-blue-700 hover:text-blue-500">
