...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="w-full text-center lg:text-left text-gray-700 text-3xl font-medium">Изображения</h3>
            ...
            <div class="pag flex flex-col items-center -my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200 mb-2">
                    <div class="grid min-w-full grid-cols-[repeat(16,_minmax(0,_1fr))]">
                        <a href="{{ route("admin.images.sort", ["id", isset($direction) && $direction == "asc" && $column == "id" ? "desc" : "asc"]) }}" class="flex items-center justify-center md:justify-start col-span-2 md:col-span-3 md:px-6 text-center md:text-left py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>ID</span>
                        ...
                        <a href="{{ route("admin.images.sort", ["source", isset($direction) && $direction == "asc" && $column == "source" ? "desc" : "asc"]) }}" class="flex items-center justify-center md:justify-start col-span-6 md:col-span-6 xl:col-span-6 md:px-6 text-center md:text-left py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>Изображение</span>
                            @if(isset($direction) && isset($column) && $column == "source")
                                <svg class="w-3 h-3 @if($direction == "desc") rotate-180 @endif" fill="#6b7280" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M21,21H3L12,3Z"></path></g></svg>
                            @endif
                        </a>
                        <a href="{{ route("admin.images.sort", ["title", isset($direction) && $direction == "asc" && $column == "title" ? "desc" : "asc"]) }}" class="hidden md:flex items-center md:col-span-4 xl:col-span-4 px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span>Товар</span>
                        ...
                        <div class="col-span-8 md:col-span-3 xl:col-span-3 text-right px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route("admin.images.create") }}" class="text-blue-700 hover:text-blue-500">Добавить</a>
                        </div>
                        @foreach($images as $image)
                            <div class="odd:bg-white even:bg-slate-50 text-sm leading-5 text-gray-900 col-[1_/_span_16] grid grid-cols-[repeat(16,_minmax(0,_1fr))]">
                                <div class="col-span-2 md:col-span-3 md:px-6 text-center md:text-left py-4 font-medium">{{ $image->id }}</div>
                                <div class="col-span-6 md:col-span-6 xl:col-span-6 md:px-6 text-center md:text-left py-4 font-medium truncate">{{ $image->source }}</div>
                                <div class="hidden md:block md:col-span-4 xl:col-span-4 px-6 py-4">{{ $image->title }}</div>
                                <div class="col-span-8 md:col-span-3 xl:col-span-3 px-6 py-4 text-right flex flex-row md:flex-col justify-end gap-2">
                                    <a href="{{ route("admin.images.edit", $image->id) }}" class="text-blue-700 hover:text-blue-500">
